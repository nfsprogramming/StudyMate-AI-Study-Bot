...
class GoogleClassroomIntegration:
    ...
    def authenticate(self, manual=False):
        ...
        Args:
            manual: If True, returns the auth URL instead of running a local server.
        """
        ...
        env_token_json = os.environ.get('GOOGLE_TOKEN')
        if env_token_json:
            try:
                token_data = json.loads(env_token_json)
                self.creds = Credentials.from_authorized_user_info(token_data, SCOPES)
            except Exception as e:
                print(f"Error loading GOOGLE_TOKEN from env: {e}")

        # 2. Try to load existing token from file
        if (not self.creds or not self.creds.valid) and os.path.exists(token_path):
            try:
        ...
        # 4. If still no valid credentials, we need to start a new flow
        if not self.creds or not self.creds.valid:
            # Check for credentials (client secrets)
            ...
            if manual:
                # Return the flow so the caller can get the auth URL
                return flow, "Manual authentication required"
                
            try:
                # This only works if you have access to a browser on the machine
                # and can access the loopback interface. Fail gracefully for remote servers.
                self.creds = flow.run_local_server(port=0, access_type='offline', timeout=10)
            except Exception as e:
                return False, f"Could not start local auth server: {str(e)}. Use manual authentication."
    ...
    def authenticate_with_token(self, token: str):
        """Authenticate directly using a Bearer token from the frontend"""
        try:
            ...
            return False, f"Token authentication error: {str(e)}"
    ...
    def get_auth_url(self):
        ...
        # Set redirect URI to 'urn:ietf:wg:oauth:2.0:oob' for manual copy-paste
        # Note: 'urn:ietf:wg:oauth:2.0:oob' is being deprecated by Google.
        # Newer flows use a localhost redirect or a specific web redirect.
        # For our purposes, we'll use a localhost flow but capture the URL.
        flow.redirect_uri = 'http://localhost:8080/' # Standard local redirect
        auth_url, _ = flow.authorization_url(prompt='consent', access_type='offline')
        return auth_url, None

    def complete_auth(self, code):
        """Complete the authentication process with a code"""
        try:
            backend_dir = os.path.dirname(os.path.abspath(__file__))
            env_creds_json = os.environ.get('GOOGLE_CREDENTIALS')
            ...
            return False, f"Failed to complete authentication: {str(e)}"
    ...
    def _format_quiz_for_classroom(self, quiz_data):
        """Format the quiz JSON into a readable markdown/text format"""
        description = "📝 StudyMate AI Generated Quiz\n\n"
        description += "Please answer the following questions to test your knowledge:\n\n"
        
        for i, q in enumerate(quiz_data):
            description += f"Question {i+1}: {q.get('question', '')}\n"
            options = q.get('options', [])
            for j, opt in enumerate(options):
                description += f"   {'ABCDEFGHIJKLMNOPQRSTUVWXYZ'[j]}. {opt}\n"
            description += "\n"
